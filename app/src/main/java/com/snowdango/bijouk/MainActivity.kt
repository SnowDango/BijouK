...
class MainActivity : ComponentActivity() {
                        ...
                        DeviceScreen(
                            onClickDevice = { data ->
                                navController.navigate(
                                    Route.NOW_PLAY(id = data.id, name = data.name, baseUrl = data.baseUrl, token = data.token)
                                )
                            }
                        )
                    }
    ...
    sealed class Route {
        ...
        object DEVICE: Route()
        @Serializable
        data class NOW_PLAY(val id: Long, val name: String, val baseUrl: String, val token: String): Route()
    }
}